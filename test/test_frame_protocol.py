...
class TestFrameDecoder(object):
    def _single_frame_test(self, client_side, frame_bytes, opcode, payload,
                           frame_finished, message_finished):
        decoder = fp.FrameDecoder(client_side=client_side)
    ...
    def _split_frame_test(self, client_side, frame_bytes, opcode, payload,
    ...
    def _split_message_test(self, client_side, frame_bytes, opcode, payload, split):
        decoder = fp.FrameDecoder(client_side=client_side)
    ...
    def _parse_failure_test(self, client_side, frame_bytes, close_reason):
        decoder = fp.FrameDecoder(client_side=client_side)
        with pytest.raises(fp.ParseFailed) as excinfo:
            decoder.receive_bytes(frame_bytes)
    ...
    def test_partial_message_frames(self):
        ...
        decoder = fp.FrameDecoder(client_side=True)
...
class TestFrameDecoderExtensions(object):
    ...
    def test_rsv_bit(self):
        ...
        decoder = fp.FrameDecoder(client_side=True, extensions=[ext])
    ...
    def test_outbound_handling_single_frame(self):
        ext = self.FakeExtension()
        proto = fp.FrameProtocol(client_side=False, extensions=[ext])
        payload = u'😃😄🙃😉'
...
class TestFrameProtocolReceive(object):
    def test_long_text_message(self):
        ...
        protocol = fp.FrameProtocol(client_side=True, extensions=[])
...
class TestFrameProtocolSend(object):
    def test_simplest_possible_close(self):
        proto = fp.FrameProtocol(client_side=False, extensions=[])
        data = proto.close()
    ...
    def test_client_side_side_masking_short_frame(self):
        proto = fp.FrameProtocol(client_side=True, extensions=[])
        payload = b'x' * 125
        data = proto.send_data(payload, fin=True)
        assert data[0] == 0x82
