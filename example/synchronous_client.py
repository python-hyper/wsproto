...
from wsproto.connection import ConnectionState, ConnectionType, WSConnection
...
def wsproto_demo(host, port):
    ...
    assert ws._state == ConnectionState.OPEN
    ws.close(code=1000, reason='sample reason')
    assert ws._state == ConnectionState.CLOSING
    ...
    net_recv(ws, conn)
    assert ws._state == ConnectionState.CLOSED
